...
class CPT:
    ...
    ### Below: Pile axial capacity - Conventional Alpha and Beta Method ###
    def _pile_capacity_Alpha_Beta_unit_fs(self, z, su, sigma_v_e, dr_index, consider_clay):
        if consider_clay is False:  # Sand
            # Table 1 of API RP 2GEO 2021
            if dr_index == 2:  # 2nd row (med dense, sand-silt)
                beta = 0.29
            elif dr_index == 3:  # 3rd row (med dense, sand; dense, sand-silt)
                beta = 0.37
            elif dr_index == 4:  # 4th row (dense, sand; very dense, sand-silt)
                beta = 0.46
            elif dr_index == 4:  # 5th row (very dense, sand)
                beta = 0.56
            unit_shaft_friction = beta * sigma_v_e
            ...
            if alpha > 1:
                alpha = 1
            unit_shaft_friction = alpha * su

        return unit_shaft_friction

    def _pile_capacity_Alpha_Beta_unit_Qb(self, z, sigma_v_e, dr_index, consider_clay):
        # Note: z shall be at pile tip level
        if consider_clay is False:  # Sand
            # Table 1 of API RP 2GEO 2021
            if dr_index == 2:  # 2nd row (med dense, sand-silt)
                nq = 12
                ...
                nq = 50
            unit_end_bearingn = nq * sigma_v_e

        else:  # Clay
            unit_end_bearing = 9 * su

        return unit_end_bearing
    ### Above: Pile axial capacity - Conventional Alpha and Beta Method ###
    
    ### Below: Pile axial capacity - API CPT-based method 1 - ICP_05 ###
    def _pile_capacity_ICP_05_unit_fs(self, z, qc, pa, sigma_v_e, delta, pile, b_compression, consider_clay):
        # Note: ICP_05 is for sand only
        if consider_clay is True:
            print('This soil type is not suitable for ICP_05 method')
        ...
        c1 = u * qc * (sigma_v_e / pa) ** a
        c2 = max(depth_ratio, v) ** (-c)
        c3 = min(depth_ratio/v, 1) ** e
        unit_shaft_friction = c1 * Ar**b * c2 * tan(delta)**d * c3
        
        return unit_shaft_friction
    
    def _pile_capacity_ICP_05_unit_Qb(self, qc_av, Dcpt, pile, consider_clay):
        # Note: z shall be at pile tip level
        # Note: ICP_05 is for sand only
        if consider_clay is True:
            print('This soil type is not suitable for ICP_05 method')
        ...
        Do = pile.dia_out
        unit_end_bearing = qc_av * (0.5 - 0.25 * np.log10(Do / Dcpt))
        if unit_end_bearing < 0.15 * qc_av:
            unit_end_bearing = 0.15 * qc_av

        # Note: the unit end bearing is for plugged case. For unplugged case, a separated equation will be used for end bearing capacity
        return unit_end_bearing
    ### Above: Pile axial capacity - API CPT-based method 1 - ICP_05 ###
    
    ### Below: Pile axial capacity - API CPT-based method 2 - UWA_05 ###
    def _pile_capacity_UWA_05_unit_fs(self, z, qc, pa, sigma_v_e, delta, pile, b_compression, consider_clay):
         # Note: UWA_05 is for sand only
        if consider_clay is True:
            print('This soil type is not suitable for UWA_05 method')
        ...
        e = 0
        v = 2
        u = 0.030 if b_compression else 0.022
    ...
    def _pile_capacity_UWA_05_unit_Qb(self, qc_av, pile, consider_clay):
        # Note: z shall be at pile tip level
        # Note: UWA_05 is for sand only
        if consider_clay is True:
            print('This soil type is not suitable for UWA_05 method')
        ...
        Do = pile.dia_out
        Di = pile.dia_inner
        unit_end_bearing = qc_av * (0.15 + 0.45 * (1 - (Di / Do) ** 2))

        # Note: the unit end bearing is for plugged case only
        return unit_end_bearing
    ### Above: Pile axial capacity - API CPT-based method 2 - UWA_05 ###
    
    ### Below: Pile axial capacity - API CPT-based method 3 - FUGRO_05 ###
    def _pile_capacity_FUGRO_05_unit_fs(self, z, qc, pa, sigma_v_e, delta, pile, b_compression, consider_clay):
         # Note: FUGRO_05 is for sand only
        if consider_clay is True:
            print('This soil type is not suitable for FUGRO_05 method')
        ...
        a = 0.05 if b_compression else 0.15
        b = 0.45 if b_compression else 0.42
        c = 0.90 if b_compression else 0.85
        d = 0
        e = 1 if b_compression else 0
        u = 0.043 if b_compression else 0.025
        v = 2*(pile.disp_ratio)**0.5
    ...
    def _pile_capacity_FUGRO_05_unit_Qb(self, qc_av, pa, pile, consider_clay):
        # Note: z shall be at pile tip level
        # Note: Fugro_05 is for sand only
        if consider_clay is True:
            print('This soil type is not suitable for FUGRO_05 method')
        ...
        Do = pile.dia_out
        Di = pile.dia_inner
        unit_end_bearing = 8.5 * pa * (qc_av / pa) ** 0.5 * (1 - Di / Do) ** 0.25

        # Note: the unit end bearing is for plugged case only
        return unit_end_bearing
    ### Above: Pile axial capacity - API CPT-based method 3 - FUGRO_05 ###

    ### Below: Pile axial capacity - API CPT-based method 4 - NGI_05 ###
    def _pile_capacity_NGI_05_unit_fs(self, z, qc, pa, sigma_v_e, pile, b_compression, consider_clay):
         # Note: NGI_05 is for sand only
        if consider_clay is True:
            print('This soil type is not suitable for NGI_05 method')

        L = pile.embedment
        Dr = 0.4 * np.ln(qc / (22 * (sigma_v_e * pa) ** 0.5))
        F_Dr = 2.1 * (Dr - 0.1) ** 1.7
        F_sig = (sigma_v_e / pa) ** 0.25

        if b_compression is True:
            unit_shaft_friction = 1.3 * (z / L) * pa * F_sig * F_Dr
        else:
            unit_shaft_friction = (z / L) * pa * F_sig * F_Dr

        if unit_shaft_friction < 0.1 * sigma_v_e:
                unit_shaft_friction = 0.1 * sigma_v_e
        
        return unit_shaft_friction

    def _pile_capacity_NGI_05_unit_Qb(self, z, qc, qc_av, pa, sigma_v_e, consider_clay, plugged):
        # z and qc are at pile tip level 
        # Note: NGI_05 is for sand only
        if consider_clay is True:
            print('This soil type is not suitable for NGI_05 method')
        ...
        Dr = 0.4 * np.ln(qc / (22 * (sigma_v_e * pa) ** 0.5))
        if plugged is True:
            unit_end_bearing = (0.7 * qc_av) / (1 + 3 * Dr **2)
        else:
            unit_end_bearing = qc

        return unit_end_bearing
    ### Above: Pile axial capacity - API CPT-based method 4 - NGI_05 ###

    ### Below: Pile axial capacity - Barry Lehane's Unified CPT Method ###
    def _pile_capacity_Unified_CPT_unit_fs(self, z, qc, Qtn, fs, qt, sigma_v_e, sigma_v_total, pile, b_compression, consider_clay):
        if consider_clay is False:
            Do = pile.dia_out
            Di = pile.dia_inner
            L = pile.embedment
            h = L - z  # distance between depth z and pile tip
            PLR = 1
            A_re = 1 - PLR * (Di / Do) ** 2

            d_ref = 0.0356
            delta_sigma_rd_e = (qc / 10) * ((qc / sigma_v_e) ** (-0.33)) * (d_ref / Do)

            c1 = max(1, (h / Do))
            sigma_rc_e = (qc / 44) * (A_re ** 0.3) * c1 ** (-0.4)
            ...
            unit_shaft_friction = f_L * (delta_sigma_rd_e + sigma_rc_e) * np.tan(29 * np.pi / 180)

        else:
            F_r = fs * 100 / (qt - sigma_v_total)
            I_z1 = Qtn - 12 * Qtn ** (-1.4 * F_r)
            if I_z1 >= 1:
                F_st = 1
            else:
                F_st = 0.5
            D_star = (Do ** 2 - Di ** 2) ** 0.5

            c2 = max(1, h / D_star)
            unit_shaft_friction = 0.07 * F_st * qt * c2 ** (-0.25)

        return unit_shaft_friction
    
    def _pile_capacity_Unified_CPT_unit_Qb(self, qt, qp, pile, consider_clay, plugged):
        if consider_clay is False:
            Do = pile.dia_out
            Di = pile.dia_inner
            PLR = 1
            A_re = 1 - PLR * (Di / Do) ** 2
            if plugged is True:
                unit_end_bearing = (0.12 + 0.38 * A_re) * qp
            else:
                unit_end_bearing = A_re * qp
        else:

            D_star = (Do ** 2 - Di ** 2) ** 0.5
            unit_end_bearing = qt * (0.2 + 0.6 * (D_star / Do) ** 2)
            # Note: the unit end bearing of clay is for plugged case only
            if plugged is False:
                print('unplugged case will not be considered for clay in Unified CPT Method')

        return unit_end_bearing
    ### Above: Pile axial capacity - Barry Lehane's Unified CPT Method ###
